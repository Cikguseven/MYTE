...
def get_dataset(lang, dataset_dir, task, tokenizer, sample_size=100, split='test'):
    ...
    with open(data_file, 'r') as f:
        examples = [parse_data_example(task, json.loads(line)) for line in f.readlines()]

    dataset = Dataset.from_list(examples)
    dataset = dataset.shuffle(seed=42).select(range(sample_size))
    dataset = dataset.map(partial(preprocess_function, tokenizer=tokenizer), desc="Running tokenizer", batched=True, batch_size=50)
...
def compute_chrf_for_task(model, tokenizer, lang, dataset_dir, task):
    ...
    # Normalize both predictions and targets consistently
    predictions, targets = normalize_targets_predictions(predictions, targets)

    # Compute chrF score
    return sacrebleu.corpus_chrf(predictions, [[t] for t in targets])
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--results_dir", required=False, default="../xtreme_up_results/mt", type=str)
